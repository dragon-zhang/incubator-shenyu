...
ELSE
    ...
    PERFORM public.dblink_exec('init_conn',  'INSERT  INTO "plugin_handle" ( plugin_id , field , label , data_type , type , sort , ext_obj ) VALUES (''' || '5' || ''', ''' || 'retryStrategy' || ''', ''' || 'retryStrategy' || ''', 2, 2, 0, ''' || '{"required":"0","defaultValue":"current","placeholder":"retryStrategy","rule":""}' || ''');');
